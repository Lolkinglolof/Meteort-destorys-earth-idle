using UnityEngine;
using System.Collections.Generic;

public class PlayerSkade : MonoBehaviour
{
    public float baseMass = 2f;

    private MeteorController controller;
    private PlayerHealth health;

    private Dictionary<int, float> knockbackCooldowns = new Dictionary<int, float>();

    [Header("Debug")]
    public bool debugImpact = true;

    void Start()
    {
        controller = GetComponent<MeteorController>();
        health = GetComponent<PlayerHealth>();
    }

    private void OnTriggerEnter2D(Collider2D other)
    {
        HandleCollisionImpact(other);
        HandleEnemyKnockbackOnly(other);
    }

    private void HandleCollisionImpact(Collider2D other)
    {
        CollisionImpact impactData = other.GetComponentInParent<CollisionImpact>();

        if (impactData == null || controller == null)
            return;

        float otherMass = impactData.objectMass;

        MeteorController otherMeteor = other.GetComponentInParent<MeteorController>();

        if (otherMeteor != null && otherMeteor != controller)
        {
            otherMass = otherMeteor.currentLiveMass;
        }

        Vector2 hitDirection = (transform.position - other.transform.position).normalized;

        controller.ApplyImpact(
            impactData.speedPenalty,
            impactData.impactForce,
            impactData.breakGrabOnHit,
            otherMass,
            hitDirection
        );
    }

    private void HandleEnemyKnockbackOnly(Collider2D other)
    {
        if (!other.CompareTag("Enemy") &&
            !other.CompareTag("SmallDebris") &&
            !other.CompareTag("RareMeteor"))
        {
            return;
        }

        Meteor2022WJ1 enemy = other.GetComponentInParent<Meteor2022WJ1>();

        if (enemy == null)
            return;

        int instanceID = enemy.gameObject.GetInstanceID();

        if (knockbackCooldowns.ContainsKey(instanceID) &&
            Time.time < knockbackCooldowns[instanceID] + 0.5f)
        {
            return;
        }

        float currentMultiplier = health != null ? health.currentMassMultiplier : 1f;
        float rawMass = baseMass * currentMultiplier;

        Vector3 playerVelocity = controller != null ? controller.CurrentVelocity : Vector3.zero;
        Vector3 enemyVelocity = Vector3.zero;

        Rigidbody2D enemyRb = other.GetComponentInParent<Rigidbody2D>();

        if (enemyRb != null)
        {
            enemyVelocity = new Vector3(enemyRb.linearVelocity.x, enemyRb.linearVelocity.y, 0f);
        }

        float impactSpeed = Vector3.Distance(playerVelocity, enemyVelocity);
        float speedBonus = Mathf.Clamp(impactSpeed * 0.05f, 0f, 1.0f);
        float knockbackPower = rawMass * (1f + speedBonus);

        if (knockbackPower > 0.5f)
        {
            if (debugImpact)
            {
                Debug.Log(
                    "<color=orange>[PLAYERSKADE]</color> Knockback only | enemy=" +
                    enemy.name +
                    " | power=" + knockbackPower.ToString("F1") +
                    " | impactSpeed=" + impactSpeed.ToString("F1") +
                    " | NO TakeDamage here"
                );
            }

            // Vigtigt:
            // PlayerSkade må IKKE kalde enemy.TakeDamage().
            // MeteorCollision.cs styrer alt damage + coins.
            enemy.ApplyKnockback(transform.position, knockbackPower * 0.5f);

            knockbackCooldowns[instanceID] = Time.time;
        }
    }
}